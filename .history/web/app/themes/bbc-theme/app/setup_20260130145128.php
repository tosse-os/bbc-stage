...
/**
 * Dashboard-Logout ohne wp-login.php
 * - kein Bestätigungsdialog
 * - sofortiger Logout
 * - Redirect auf /dashboard-login
 */

/* Eigene Logout-URL registrieren */
add_action('init', function () {
    add_rewrite_rule(
        '^dashboard-logout/?$',
        'index.php?dashboard_logout=1',
        'top'
    );
});
...
add_filter('query_vars', function ($vars) {
    $vars[] = 'dashboard_logout';
    return $vars;
});

/* Logout ausführen und sauber weiterleiten */
add_action('template_redirect', function () {
    if (get_query_var('dashboard_logout')) {
        wp_logout();
