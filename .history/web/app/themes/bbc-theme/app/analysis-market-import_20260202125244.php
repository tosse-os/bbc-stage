 ...
 */
function analysis_market_import_phase2(array $item, array $map, array &$log)
{
  if (!empty($item['parent_id'])) {

    if (!isset($map[$item['id']], $map[$item['parent_id']])) {
      $log[] = "❌ Parent fehlt für {$item['name']} ({$item['id']})";
    } else {
      wp_update_term($map[$item['id']], 'analysis_market', [
  ...
  if (!empty($item['children'])) {
    foreach ($item['children'] as $child) {
      analysis_market_import_phase2($child, $map, $log);
    }
  }
}
