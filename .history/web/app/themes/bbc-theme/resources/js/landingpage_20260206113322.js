...
function initSmoothScroll(duration = 700) {
  document.querySelectorAll('a[href^="#"]').forEach(link => {
    link.addEventListener('click', function (e) {
      ...
      const targetY = absoluteY - customOffset;
  ...
  const observer = new IntersectionObserver(
    entries => {
      ...
      visibleEntries.forEach((entry, localIndex) => {
        ...
        const delay = Number(el.dataset.revealDelay ?? localIndex * 250)
...
/**
 *
 * @returns Hero image effect
 */

function initHeroFloat() {
  const el = document.querySelector('.hero-phone-float')
  if (!el) return

  function update() {
    const y = Math.min(window.scrollY / 12, 12)
...
/* Scroll-Spy für Landingpage Navigation mit Debug-Ausgaben */

const navLinks = Array.from(document.querySelectorAll('.nav-link'));

console.log('[ScrollSpy] navLinks gefunden:', navLinks.length);

if (!navLinks.length) {
  console.warn('[ScrollSpy] Keine .nav-link gefunden – Abbruch');
} else {
  const sections = navLinks
    .map(link => {
      ...
      console.log('[ScrollSpy] Link → Section', target, section ? 'OK' : 'NICHT GEFUNDEN');

      return section;
    })
    .filter(Boolean);

  console.log('[ScrollSpy] Gültige Sections:', sections.map(s => s.id));

  const updateActiveNav = () => {
    const triggerPoint = window.innerHeight * 0.5;

    console.log('--- Scroll ---');
    console.log('[ScrollSpy] TriggerPoint:', triggerPoint);
    ...
    console.log(
      '[ScrollSpy] Aktive Section:',
      activeSection ? `#${activeSection.id}` : 'KEINE'
    );
    ...
    navLinks.forEach(link => {
      ...
      if (isActive) {
        console.log('[ScrollSpy] Aktiv gesetzt:', href);
      }
    });
  };
