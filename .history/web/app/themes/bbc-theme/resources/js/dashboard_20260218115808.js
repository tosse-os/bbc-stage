...
function initMediaAutoTitle() {
  ...
  // Wir nutzen 'select_link', da dies exakt feuert, wenn eine Datei im Modal gewählt wurde
  acf.addAction('select_data', function (data, $el) {

    // Prüfen, ob wir im richtigen Feld (audio_file) sind
    // Wir schauen nach dem Daten-Attribut des umschließenden ACF-Feldes
    const $field = $el.closest('.acf-field');
    if ($field.data('name') !== 'audio_file') return;

    const $titleField = document.querySelector('#title');

    // Nur ausfüllen, wenn Titel leer/Auto Draft ist
    if ($titleField && (!$titleField.value || $titleField.value === 'Auto Draft' || $titleField.value === 'Titel hinzufügen')) {
      ...
      if (data && data.filename) {
        ...
        // Visuelles Feedback
        $titleField.style.transition = 'all 400ms ease';
        $titleField.style.backgroundColor = 'rgba(0, 255, 100, 0.1)';
        $titleField.style.borderColor = '#007cba';
