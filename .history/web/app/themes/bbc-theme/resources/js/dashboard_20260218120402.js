...
/*
|--------------------------------------------------------------------------
| Media Auto-Title (Live Sync)
|--------------------------------------------------------------------------
*/
function initMediaAutoTitle() {
  if (typeof acf === 'undefined') return;

  // Wir hören auf das ACF-Event, wenn eine Datei ausgewählt wurde
  acf.addAction('select_data', function (data, $el) {

    // Prüfen, ob wir im Feld 'audio_file' sind
    const $field = $el.closest('.acf-field');
    if ($field.data('name') !== 'audio_file') return;

    const $titleField = document.querySelector('#title');

    // Nur ausfüllen, wenn das Titelfeld leer ist oder den Standardtext enthält
    ...
    if ($titleField && isPlaceholder) {
      if (data && data.filename) {
        // Die Logik aus deiner Vorlage (Endung weg, Trenner ersetzen)
        let cleanTitle = data.filename.split('.').slice(0, -1).join('.')
          .replace(/[-_]/g, ' ')
          .replace(/\b\w/g, l => l.toUpperCase()); // Erster Buchstabe groß

        // Den Titel direkt im Feld anzeigen
        $titleField.value = cleanTitle;

        // Fokus auf das Titelfeld setzen, damit der User sieht, dass etwas passiert ist
        $titleField.classList.add('is-automated');

        // Optionaler visueller Effekt (Flash)
