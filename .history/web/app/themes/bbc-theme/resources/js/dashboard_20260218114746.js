...
function initMediaAutoTitle() {
  ...
  // Abbrechen, wenn wir nicht im WordPress-Backend mit ACF sind
  if (typeof acf === 'undefined') return;

  /**
   * ACF Action: Wird gefeuert, wenn sich ein Feldwert ändert.
   */
  acf.addAction('change', function ($el) {

    // Prüfen, ob das geänderte Feld das Audio-Feld ist
    if ($el.data('name') === 'audio_file') {

      const $titleField = document.querySelector('#title');

      // Nur eingreifen, wenn das Titel-Feld existiert und leer (oder Auto Draft) ist
      if ($titleField && (!$titleField.value || $titleField.value === 'Auto Draft')) {

        const selection = acf.getSelection($el);

        if (selection && selection.length > 0) {
          const filename = selection[0].filename;

          // Transformation: Endung weg, Trenner zu Leerzeichen, Großschreibung
          let cleanTitle = filename.split('.').slice(0, -1).join('.')
            .replace(/[-_]/g, ' ')
          ...
          // Kurzes visuelles Feedback (optional, passt zum "9.5/10" Look)
          $titleField.style.transition = 'background-color 300ms';
          $titleField.style.backgroundColor = 'rgba(255, 249, 196, 0.5)'; // Zartes Gelb
