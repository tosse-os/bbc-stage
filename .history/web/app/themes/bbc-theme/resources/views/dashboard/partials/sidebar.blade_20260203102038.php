@if (is_user_logged_in())
@php
$user = wp_get_current_user();
$avatar = get_avatar_url($user->ID, ['size' => 64]);
@endphp

<aside
  data-sidebar
  class="w-72 transition-all duration-300 text-slate-100 flex flex-col bg-slate-900">

  {{-- Header --}}
  <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between">

    <img
      src="{{ Vite::asset('resources/images/dashboard/bloombridge-capital-logo-v2.png') }}"
      alt="Bloombridge Capital"
      class="sidebar-logo h-10 transition-all duration-300 mx-auto">

    <button
      type="button"
      data-sidebar-toggle
      class="text-slate-400 hover:text-white ml-2">
      <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        <rect x="4" y="11" width="16" height="2" />
      </svg>
    </button>

  </div>

  {{-- Navigation --}}
  <nav class="flex-1 px-4 py-6 space-y-1 text-sm">

    <a href="/dashboard" class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-lg bg-slate-800/60">
      <span class="w-5 h-5">@include('dashboard.icons.overview')</span>
      <span class="sidebar-label font-medium">Overview</span>
    </a>

    <a href="/dashboard/reports" class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-800/50">
      <span class="w-5 h-5">@include('dashboard.icons.report')</span>
      <span class="sidebar-label font-medium">Reports</span>
    </a>

    <a href="/dashboard/settings" class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-slate-800/50">
      <span class="w-5 h-5">@include('dashboard.icons.settings')</span>
      <span class="sidebar-label font-medium">Settings</span>
    </a>

  </nav>

  {{-- Logout --}}
  <div class="px-6 py-4 border-t border-slate-800 text-sm">
    <a href="/?dashboard_logout=1" class="sidebar-item flex items-center gap-3 text-slate-400 hover:text-white">
      <span class="w-5 h-5">@include('dashboard.icons.logout')</span>
      <span class="sidebar-label font-medium">Logout</span>
    </a>
  </div>

  {{-- Profil --}}
  <div class="px-6 py-4 border-t border-slate-800">

    <div class="flex items-center gap-3 sidebar-item">

      <img src="{{ $avatar }}" class="w-9 h-9 rounded-full object-cover">

      <div class="sidebar-label leading-tight">
        <div class="text-sm font-medium text-white">{{ $user->display_name }}</div>
        <div class="text-xs text-slate-400">View profile</div>
      </div>

    </div>

  </div>

</aside>
@endif
