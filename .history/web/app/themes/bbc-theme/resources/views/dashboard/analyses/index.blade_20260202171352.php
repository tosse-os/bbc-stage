<section class="max-w-7xl">
  ...
  <header class="mb-6 flex items-center justify-between">
    ...
    <div class="inline-flex rounded-lg border bg-white overflow-hidden text-sm">
      <a
        href="{{ request()->fullUrlWithQuery(['view' => 'default']) }}"
        class="px-3 py-2 {{ $view === 'default' ? 'bg-slate-200' : 'hover:bg-slate-100' }}">
        ▦
      </a>
      <a
        href="{{ request()->fullUrlWithQuery(['view' => 'grid']) }}"
        class="px-3 py-2 {{ $view === 'grid' ? 'bg-slate-200' : 'hover:bg-slate-100' }}">
        ⬚⬚
      </a>
      <a
        href="{{ request()->fullUrlWithQuery(['view' => 'list']) }}"
        class="px-3 py-2 {{ $view === 'list' ? 'bg-slate-200' : 'hover:bg-slate-100' }}">
        ☰
      </a>
    </div>
  </header>
  ...
  <div class="bg-white rounded-xl shadow-sm">
    ...
    <div class="divide-y">
      ...
      <div class="grid grid-cols-[160px_140px_1fr_140px_160px] px-6 py-5 gap-4">
