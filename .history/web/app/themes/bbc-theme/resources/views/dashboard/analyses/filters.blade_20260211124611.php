...
{{--
    STICKY WRAPPER
    'top-0' macht ihn sticky. '-mx-4' sorgt mobil für Kante-zu-Kante Look.
--}}
<div class="sticky top-0 z-40 -mx-4 px-4 md:relative md:mx-0 md:px-0 md:z-10 mb-6 transition-all duration-300" id="sticky-filter-wrapper">
  <form
    method="get"
    id="analysis-filters"
    data-filter-panel
    ...
    {{-- HEADER: Extrem kompakt (py-1.5 mobil) --}}
    ...
    {{-- CONTENT: Mobil gap-2 für noch weniger Höhe --}}
    <div
      data-filter-content
      ...
      <div class="w-full md:w-auto">
        ...
        <div class="flex items-center rounded-lg border border-brand-primary/20 bg-white/50 overflow-hidden">
          <input type="date" name="date_from" value="{{ $dateFrom }}" class="w-full bg-transparent px-2 py-1.5 text-xs outline-none">
          <div class="h-4 w-px bg-brand-primary/10"></div>
          <input type="date" name="date_to" value="{{ $dateTo }}" class="w-full bg-transparent px-2 py-1.5 text-xs outline-none">
        </div>
      </div>

      {{-- Buttons --}}
      <div class="flex items-center gap-2 pt-1 md:pt-0 md:ml-auto">
        <a href="/dashboard" class="flex items-center justify-center h-8 w-8 rounded-lg bg-brand-primary/10 text-brand-primary">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
        </a>
        <button type="submit" class="flex-1 md:hidden bg-brand-primary text-white py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wide">
          Anwenden
        </button>
      </div>
    </div>
