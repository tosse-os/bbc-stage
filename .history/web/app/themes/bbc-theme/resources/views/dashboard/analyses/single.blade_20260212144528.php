...
<article class="max-w-6xl mx-auto space-y-8">
  ...
  <header class="bg-white/80 backdrop-blur rounded-xl px-6 py-5 flex items-start justify-between gap-8 shadow-sm">

    <div class="space-y-3">

      {{-- ASSET TITLE – klar dominierend --}}
      ...
      <div class="flex flex-wrap items-center gap-8 text-sm text-slate-300">

        @if ($reportWkn)
        <div class="relative flex items-center gap-2 group">
          <span class="font-medium text-slate-200">WKN {{ $reportWkn }}</span>

          <button
            type="button"
            data-copy="{{ $reportWkn }}"
            class="copy-btn relative text-slate-400 hover:text-white transition">
        ...
        @if ($reportIsin)
        <div class="relative flex items-center gap-2 group">
          <span class="font-medium text-slate-200">ISIN {{ $reportIsin }}</span>
    ...
    <a
      ...
      class="inline-flex items-center gap-2 px-4 py-2 rounded-lg
           ...
           hover:bg-slate-600 transition text-sm whitespace-nowrap">
      ← Übersicht
    </a>
