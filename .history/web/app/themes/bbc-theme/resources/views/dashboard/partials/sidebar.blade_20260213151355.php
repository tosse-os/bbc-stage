...
{{-- ACTION BAR: Language, Theme & Collapse --}}
<div class="mt-6 px-4 flex items-center justify-center gap-2">

  {{-- Language Selector --}}
  @php($langs = pll_the_languages(['raw' => 1]))
  @if(!empty($langs))
  <div class="flex rounded-lg border border-[var(--border-color)] bg-[var(--surface-glass)] backdrop-blur-sm overflow-hidden text-[10px] font-bold tracking-tight w-20 shrink-0">
    @foreach($langs as $lang)
    <a
      href="{{ $lang['url'] }}"
      class="flex-1 text-center py-2 transition
            {{ $lang['current_lang']
  ...
  <button
    ...
    class="flex items-center justify-center w-9 h-9 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800/50 transition shrink-0"
  ...
  <button
    ...
    class="flex items-center justify-center w-9 h-9 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800/50 transition shrink-0 group">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 transition-transform duration-300 ease-in-out" data-sidebar-icon>
