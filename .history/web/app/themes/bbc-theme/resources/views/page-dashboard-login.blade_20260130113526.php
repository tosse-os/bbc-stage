...
<body class="dashboard-login min-h-screen flex items-center justify-center text-slate-100 bg-slate-950">
  ...
  <section class="relative w-full max-w-md mx-auto px-4">
    <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl px-8 py-12 shadow-2xl ring-1 ring-white/5">
      ...
      <form method="post" action="{{ esc_url(admin_url('admin-post.php')) }}" class="space-y-6">
        ...
        <div>
          ...
          <input type="email" name="email" required placeholder="name@beispiel.de"
            class="w-full rounded-lg bg-white/10 border border-white/10 px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-primary focus:bg-white/20 transition-all">
        </div>
