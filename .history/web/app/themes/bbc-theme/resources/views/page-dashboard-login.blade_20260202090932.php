...
<section class="relative w-full max-w-md mx-auto px-4">
  <div class="backdrop-blur-xl bg-white/[0.03] border border-white/10 rounded-2xl px-8 py-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] ring-1 ring-white/10 overflow-hidden relative">
    ...
    <form method="post" action="{{ esc_url(admin_url('admin-post.php')) }}" class="space-y-6">
      ...
      <button type="submit"
        class="w-full py-4 rounded-lg bg-brand-primary
         {{-- Hover-Effekte: Schatten-Glow, Aufhellung und sanfte Skalierung --}}
         hover:bg-brand-primaryHover hover:shadow-[0_0_20px_rgba(var(--brand-primary-rgb,0,255,255),0.4)]
         hover:-translate-y-0.5
         {{-- Haptisches Feedback beim Klicken --}}
         active:scale-[0.98]
         {{-- Weiche Übergänge für alle Eigenschaften --}}
         transition-all duration-300
         {{-- Typografie --}}
         font-black text-xs text-white uppercase tracking-[0.2em] mt-2 shadow-lg shadow-brand-primary/20">
        Anmelden
      </button>
    </form>
