...
function shouldExcludeAppEntry(name) {
  return (
    name.startsWith('Dashboard') ||
    name.toLowerCase().startsWith('analysis')
  );
}
...
function getFilesRecursive(dir) {
  ...
  entries.forEach(entry => {
    if (shouldExcludeAppEntry(entry.name)) {
      return;
    }
...
if (fs.existsSync(appPath)) {
  collectedFiles = collectedFiles.concat(getFilesRecursive(appPath));
}
