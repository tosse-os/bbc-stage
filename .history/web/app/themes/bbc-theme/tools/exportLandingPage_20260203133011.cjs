...
function isExcludedAppPath(fullPath) {
  const rel = path.relative(appPath, fullPath).replace(/\\/g, '/');
  return (
    rel.startsWith('dashboard') ||
    rel.startsWith('analysis')
  );
}

function getFilesRecursive(dir, excludeFn = null) {
  let results = [];
  ...
  entries.forEach(entry => {
    ...
    if (excludeFn && excludeFn(fullPath)) {
      return;
    }

    if (entry.isDirectory()) {
      results = results.concat(getFilesRecursive(fullPath, excludeFn));
    }
...
function isExcludedLayoutFile(filePath) {
  return path.basename(filePath).startsWith('dashboard');
}
...
if (fs.existsSync(layoutsPath)) {
  collectedFiles = collectedFiles.concat(
    getFilesRecursive(layoutsPath)
      .filter(f => f.endsWith('.blade.php'))
      .filter(f => !isExcludedLayoutFile(f))
  );
}
...
if (fs.existsSync(appPath)) {
  collectedFiles = collectedFiles.concat(
    getFilesRecursive(appPath, isExcludedAppPath)
  );
}
